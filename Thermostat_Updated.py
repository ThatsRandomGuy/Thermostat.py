...
##
## Adding import for digital output device
##
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        redLight.pulse()  # Update the red light to fade in and out
    ...
    def on_exit_heat(self):
        # Turn off the red light when exiting the heat state
        redLight.off()
    ...
    def on_enter_cool(self):
        blueLight.pulse()  # Update the blue light to fade in and out

        fan.on() # Update the fan to turn on
    ...
    def on_enter_off(self):
        # Turn off both the red and blue lights when entering the off state
        redLight.off()
        blueLight.off()
    ...
    def updateLights(self):
        ...
        if self.current_state == self.heat:
            if temp < self.setPoint:
                redLight.pulse()  # Red LED fades in and out
            else:
                redLight.on()  # Red LED solid
        elif self.current_state == self.cool:
            if temp > self.setPoint:
                blueLight.pulse()  # Blue LED fades in and out
                fan.on() # Turn fan on when needed
            else:
                blueLight.on()  # Blue LED solid
                fan.off() # Turn fan off when not needed
    ...
    def setupSerialOutput(self):
        # Create a comma-delimited string with the current state, temperature, and setpoint
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                # Display the current state and setpoint
            ...
            if((counter % 30) == 0):
                # Send the current state information to the TemperatureServer
                ser.write(tsm.setupSerialOutput().encode('utf-8'))
